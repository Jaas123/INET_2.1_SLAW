...
void Radio::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        rs.setBitrate((double)par("bitrate"));
        rs.setChannelNumber((int)par("channelNumber"));
        rs.setRadioId(this->getId());
        thermalNoise = FWMath::dBm2mW((double)par("thermalNoise"));
        sensitivity = FWMath::dBm2mW((double)par("sensitivity"));
...
void Radio::getSensitivityList(cXMLElement* xmlConfig)
{
    ...
    for (cXMLElementList::const_iterator it = sensitivityXmlList.begin(); it != sensitivityXmlList.end(); it++)
    {
        ...
        SensitivityList::iterator _it = sensitivityList.find(0.0);
        if (_it == sensitivityList.end())
        {
            sensitivityList[0] = FWMath::dBm2mW(par("sensitivity").doubleValue());
        }
    } // end iterator loop
